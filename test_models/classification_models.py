...
def main(X,y):
    ...
    for name, acc, err in result_list:
        print(f'{name} accuracy : {round(acc*100,1)}% , mistakes : {err}')
        if acc > highest_acc:
            highest_acc = acc
            highest_name = name
            highest_mis = err
    ...
    if len(multiple_highest) > 1 :
        ...
        for i in multiple_highest :
            highest_name += f' {i} |'
    print(f'Highest accuracy {highest_name} : {round(highest_acc*100,1)}% , mistakes : {highest_mis}')
    print(f'Lowest accuracy {lowest_name} : {round(lowest_acc*100,1)}% , mistakes : {lowest_mis}')
